...
inline const Vector6 kThrusterForceDrift{
    0.0459, 0.0156, -0.0464, 0.0349, 0.0434, 0.0179};

// Additive bias [N] for each thruster.
inline const Vector6 kThrusterForceOffset{
    2.5774, 2.4313, -1.0777, 1.5548, -3.2881, 2.0605};

inline Vector6 applyThrusterForceError(const Vector6& commanded_force) {
  Vector6 out{};
  for (std::size_t i = 0; i < 6; ++i) {
    out(i) = (static_cast<Real>(1.0) + kThrusterForceDrift(i)) * commanded_force(i) +
             kThrusterForceOffset(i);
  }
  return out;
}
...
inline Vector6 removeThrusterForceError(const Vector6& measured_force) {
  Vector6 out{};
  for (std::size_t i = 0; i < 6; ++i) {
    const Real scale = static_cast<Real>(1.0) + kThrusterForceDrift(i);
    if (std::abs(scale) < static_cast<Real>(1e-9)) {
      throw std::runtime_error("removeThrusterForceError: invalid drift scale");
    }
    out(i) = (measured_force(i) - kThrusterForceOffset(i)) / scale;
  }
  return out;
}

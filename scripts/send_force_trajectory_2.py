...
Publish a two-segment desired wrench trajectory:
  ...
  Segment 2 : triangle-wave wrench, each frequency in FREQ_LIST runs for exactly
              one period (T = 1/f s), then moves to the next frequency.

Triangle wave parameters (configurable):
  W_AMP          : amplitude per axis (peak deviation from W_CONST)
  FREQ_LIST      : list of frequencies (Hz), one period each
  TRIANGLE_DUTY  : rising-edge fraction of period in (0, 1); 0.5 = symmetric

Topic : desired_wrench  (geometry_msgs/WrenchStamped)

Usage:
    rosrun sensorless_force_control send_force_trajectory_2.py
"""
...
# 第一段：常值力，持续 T1 秒
W_CONST = [0.0, 5.0, 0.0, 0.0, 0.0, 0.0]   # [fx, fy, fz, tx, ty, tz]  N / N·m
T1      = 50.0                                # 第一段时长 (s)

# 第二段：三角波信号，每个频率持续恰好一个周期 (T = 1/f)
W_AMP          = [0.0, 6.0, 0.0, 0.0, 0.0, 0.0]   # 各轴幅值 (N / N·m)
FREQ_LIST      = [0.02, 0.04, 0.06]   # Hz
TRIANGLE_DUTY  = 0.5    # 上升沿占比 (0, 1)；0.5 = 对称三角波
...
def build_force_trajectory():
    ...
    # 第二段：每个频率恰好一个完整周期（三角波）
    duty = float(TRIANGLE_DUTY)
    duty = max(1e-6, min(1.0 - 1e-6, duty))  # clamp 到 (0, 1)
    for freq in FREQ_LIST:
        period = 1.0 / freq
        steps  = max(1, int(period * PUBLISH_HZ))
        for i in range(steps):
            phase = i / steps          # 归一化相位 [0, 1)
            if phase < duty:
                val = 2.0 * phase / duty - 1.0          # -1 → +1
            else:
                val = 1.0 - 2.0 * (phase - duty) / (1.0 - duty)  # +1 → -1
            rows.append(const + amp + amp * val)
...
def run():
    ...
    for idx, wrench in enumerate(traj):
        ...
        if idx in boundaries:
            ...
            rospy.loginfo("Seg2 [%d/%d]: triangle freq = %.3f Hz, period = %.2f s, duty = %.2f",
                          fi + 1, len(FREQ_LIST), FREQ_LIST[fi], 1.0 / FREQ_LIST[fi], TRIANGLE_DUTY)

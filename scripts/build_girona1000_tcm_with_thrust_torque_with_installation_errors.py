...
def position_to_tcm(rpy,xyz)  -> np.ndarray:
  ...
  tcm = np.zeros((6,n))
  z = np.array([-1.0,0,0])
  tcm_thruster_torque = np.zeros((6,n))
  # torque = r X F
  for i in range(n):
    ...
    tcm[0:3,i] = axes
    tcm[3:6,i] = S(r)@axes
    tcm_thruster_torque[3:6,i] = -1.0 * axes
  return tcm, tcm_thruster_torque
...
def main() -> None:
  ...
  print(rpy)
  print(xyz)
  # Add random installation-position error on xyz:
  # random direction, magnitude in [0, 0.01 cm] = [0, 1e-4 m].
  rng = np.random.default_rng()
  max_pos_error_m = 1e-4
  noise_xyz = np.zeros_like(xyz)
  for i in range(xyz.shape[1]):
    ...
    magnitude = rng.uniform(0.0, max_pos_error_m)
    noise_xyz[:, i] = direction * magnitude
  xyz = xyz + noise_xyz

  # Add random installation-angle error on rpy:
  # random direction, magnitude in [0, 5 deg].
  max_rpy_error_rad = np.deg2rad(5.0)
  noise_rpy = np.zeros_like(rpy)
  for i in range(rpy.shape[1]):
    ...
    if norm < 1e-12:
      ...
      direction = direction / norm
    magnitude = rng.uniform(0.0, max_rpy_error_rad)
    noise_rpy[:, i] = direction * magnitude
  rpy = rpy + noise_rpy
  ...
  print(rpy)
  print(xyz)
  tcm_thruster_force, tcm_thruster_torque = position_to_tcm(rpy,xyz)
  print(tcm_thruster_force)
  print(tcm_thruster_torque)
  Kt = 0.48
  Kq = 0.05
  D  = 0.18
  tcm = tcm_thruster_force + Kq/Kt * D * tcm_thruster_torque
  ...
  # Save to yaml
  yaml_path = "/home/sia/girona_ws/src/sensorless_force_control/config/control/tcm_with_thruster_torque_with_installation_error.yaml"

...
typename ManipulatorBase<Dof>::Jacobian ManipulatorBase<Dof>::jacobian() const {
  ...
  // calculate joint origin wrt base frame (for arm, base frame is the world frame, inertia frame)
  for (std::size_t i = 0; i < kDof; ++i) {
    t = t * dhTransform(dh_params_[i], state_.q[i]);
  ...
  HomogeneousMatrix t_tip = t * t_tool_linkend_;

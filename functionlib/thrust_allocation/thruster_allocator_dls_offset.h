#ifndef SFC_THRUST_ALLOCATION_THRUSTER_ALLOCATOR_DLS_OFFSET_H_
#define SFC_THRUST_ALLOCATION_THRUSTER_ALLOCATOR_DLS_OFFSET_H_
...
class ThrusterAllocatorDlsOffset : public ThrusterAllocatorBase<Thrusters> {
 public:
  void setAllocationMatrix(const Matrix<6, Thrusters>& allocation) override {
    this->allocation_ = allocation;
  }
  void setHorizontalOffset(const Vector<Thrusters>& offset){
    this->offset_ = offset;
  }
  ...
  private:
    Vector<Thrusters> offset_{};
};

}  // namespace sfc

#endif  // SFC_THRUST_ALLOCATION_THRUSTER_ALLOCATOR_DLS_OFFSET_H_
